const DbColumn = require ('./DbColumn.js')

class DbRelation {

	constructor (o) {
	
		for (const k of ['columns', 'pk']) if (!(k in o)) throw Error (`${k} is not defined`)
		
		for (const k in o) this [k] = o [k]
		
		const {columns} = this; if (typeof columns !== 'object') throw Error (`columns musbe specified as ab object`)

		for (const [name, src] of Object.entries (columns)) {

			const column = new DbColumn (src)
					
			column.name = name
			column.relation = this

			columns [name] = column

		}
		
		if (!Array.isArray (this.pk)) this.pk = [this.pk]
		
		for (const name of this.pk) {
		
			if (!(name in columns)) throw Error (`PK column not defined: ${name}`)
			
			columns [name].nullable = false
			
		}

	}
	
}

module.exports = DbRelation