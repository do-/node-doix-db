...
class DbLang {
	...
	static FMT_ISO_DATE = new Intl.DateTimeFormat ('sv', {
		year: 'numeric',
		month: '2-digit',
		day: '2-digit',
	})

	static FMT_ISO_TIMESTAMP = new Intl.DateTimeFormat ('sv', {
		year: 'numeric',
		month: '2-digit',
		day: '2-digit',
		hour: '2-digit',
	...
	* genDDL () {
	...
	toCsvNull (column) {
		...
		const value = column.default; if (typeof value !== 'string') throw Error ('NULL value is not allowed for ' + column.name)

		return this.toCsv (value, column)

	}

	toCsvNumber (value, column) {

		const n = Number (value); if (Number.isNaN (n)) throw Error (column.name + ' must be a number, received ' + value)
		...
		switch (n) {
			...
			case Number.NEGATIVE_INFINITY:
				return ''
			default:
				if (column.typeDef instanceof DbTypeArithmeticFixed) return n.toFixed (column.scale)
				return n.toString ()
		}

	}

	toCsvBoolean (value, column) {

		return value ? '1' : '0'

	}

	toCsvDateString (value, column) {

		const {length} = value; if (length < 10) throw Error (column.name + ' must be a date, received ' + value)

		return value.slice (0, 10)
		
	}
	
	toCsvDate (value, column) {

		if (typeof value === 'string') return this.toCsvDateString (value, column)

		if (value instanceof Date) return DbLang.FMT_ISO_DATE.format (value)

		throw Error (column.name + ' must be a date, received ' + value)

	}

	toCsvTimestampString (value, column) {

		const {length} = value; if (length < 19) throw Error (column.name + ' must be a timestamp, received ' + value)
	
		if (!('size' in column)) {

			column.size = 19

			if (column.scale > 0) column.size += (column.scale + 1)

		}
	
		return value.slice (0, column.size)
		
	}

	toCsvTimestamp (value, column) {

		if (value instanceof Date) {
			
			value = DbLang.FMT_ISO_TIMESTAMP.format (value)

			if (column.scale > 0) value = value.replace (',', '.')

		}

		if (typeof value === 'string') return this.toCsvTimestampString (value, column)

		throw Error (column.name + ' must be a timestamp, received ' + value)

	}

	toCsv (value, column) {

		if (value == null) return this.toCsvNull (column)

		if (!('typeDef' in column)) column.setLang (this); const {typeDef} = column

		if (typeDef instanceof DbTypeCharacter) return CH_QQ + QQ_ESC.escape (String (value)) + CH_QQ

		if (typeDef instanceof DbTypeArithmetic) return this.toCsvNumber (value, column)

		if (typeDef instanceof DbTypeDate) return this.toCsvDate (value, column)

		if (typeDef instanceof DbTypeTimestamp) return this.toCsvTimestamp (value, column)

		if (typeof value === 'boolean') return this.toCsvBoolean (value, column)

		throw Error (`Cannot convert to CSV the ${column.name} value: ${value}`)
