...
class DbLang {
	...
	* genColumnDefinitionParts (column) {
	...
	parseColumnComment (o) {
		...
		const {src} = o, pos = src.lastIndexOf ('//')

		if (pos < 0) return o.type = src.trim ()

		o.type    = src.slice (0, pos).trim ()
			
		o.comment = src.slice (pos + 2).trim ()

	}

	parseColumnDefault (o) {

		const pos = o.type.indexOf ('='); if (pos < 0) return
		
		o.default = o.type.slice (pos + 1).trim ()

		o.type = o.type.slice (0, pos).trim ()

	}
	
	parseColumnPattern (o) {

		const last = o.default ? 'default' : 'type', src = o [last]
		...
		while (true) {
			...
			if (pos < 0) throw Error (`Invalid column definition: cannot find opening '/' for pattern in '${o.src}'`)

			if (pos === 0) throw Error (`Invalid column definition: the ${last} is empty in '${o.src}'`)
	...
	parseColumnRange (o) {
		...
		const begin = src.lastIndexOf ('{'); if (begin < 0) throw Error (`Invalid column definition: cannot find opening '{' for range in '${o.src}'`)

		const range = src.slice (begin + 1, -1)

		o [last] = src.slice (0, begin).trim ()

		const pos = range.indexOf ('..'); if (pos < 0) throw Error (`Invalid column definition: cannot find '..' between '{' and '}' in '${o.src}'`)
		
		const min = range.slice (0, pos).trim (); if (min.length !== 0) o.min = min

		const max = range.slice (pos + 2).trim (); if (max.length !== 0) o.max = max
	...
	parseColumnNullable (o) {
		...
		const src = o.type, {length} = src; if (length === 0) throw Error (`Invalid column definition: cannot determine type in '${o.src}'`)
	...
	parseColumnDimension (o) {
		...
		const begin = src.lastIndexOf ('('); if (begin < 0) throw Error (`Invalid column definition: cannot find opening '(' for dimension in '${o.src}'`)
		...
		const pos = dimension.indexOf (','); if (pos >= 0) {
			...
			if (!RE_INT.test (scale)) throw Error (`Invalid column definition: not a positive integer as scale in '${o.src}'`)
		...
		if (!RE_INT.test (dimension)) throw Error (`Invalid column definition: not a positive integer as column dimension in '${o.src}'`)
