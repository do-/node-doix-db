...
class DbModel extends EventEmitter {
	...
	constructor (o) {
		...
		this.schemata = new Map (); this.addSchema (null)
	...
	addSchema (name, schemaName = null) {

		this.schemata.set (name, new DbSchema ({name, schemaName, model: this}))
	...
	loadModules () {
		...
		const {defaultSchema} = this, om = new DbObjectMap (this.mapOptions)

		om.load ()

		for (const [k, v] of om) defaultSchema.add (k, v)
