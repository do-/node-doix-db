...
class DbModel extends EventEmitter {
	...
	* objects () {
		...
		for (const schema of this.schemata.values ())
			...
			for (const object of schema.map.values ())
	...
	resolveReferences () {
		...
		for (const object of this.objects ()) if (object instanceof DbRelation) {
			...
			for (const column of Object.values (object.columns)) if ('reference' in column) {
				...
				const targetSchema = targetSchemaName === undefined ? object.schema : this.getSchema (targetSchemaName); if (!targetSchema) throw Error (`Cannot resolve reference from ${column.relation.name}.${column.name}: schema "${targetSchemaName}" not found`)
	...
	assignData (data, names) {

		for (const name of names) data [name] = this.find (name).data
