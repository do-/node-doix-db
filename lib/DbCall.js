...
/*	
	async processResult () {
		...
		const {options, rows} = this, {maxRows} = options; if (maxRows === 0) return

		const scalarMode = options.rowMode === MD_SCALAR

		if (Array.isArray (rows)) {

			if (scalarMode) this.flattenArray ()

			return this.finish ()

		}

		if (scalarMode) this.flattenStream ()

		this.observeStream ()

		if (maxRows !== Infinity) await this.fetchStream ()
